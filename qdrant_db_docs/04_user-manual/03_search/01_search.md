...
[Recommendations](https://qdrant.tech/documentation/search/explore/#recommendation-api) ([local](./04_explore.md#recommendation-api))| Provide positive and negative examples  
[Discovery Search](https://qdrant.tech/documentation/search/explore/#discovery-api) ([local](./04_explore.md#discovery-api))| Guide the search using context as a one-shot training set  
[Scroll](https://qdrant.tech/documentation/manage-data/points/#scroll-points) ([local](./../01_manage-data/01_points.md#scroll-points))| Get all points with optional filtering  
[Grouping](#grouping-api)| Group results by a certain field  
[Order By](https://qdrant.tech/documentation/manage-data/points/#order-points-by-payload-key) ([local](./../01_manage-data/01_points.md#order-points-by-payload-key))| Order points by payload key  
[Hybrid Search](https://qdrant.tech/documentation/search/hybrid-queries/#hybrid-search) ([local](./03_hybrid-queries.md#hybrid-search))| Combine multiple queries to get better results  
[Multi-Stage Search](https://qdrant.tech/documentation/search/hybrid-queries/#multi-stage-queries) ([local](./03_hybrid-queries.md#multi-stage-queries))| Optimize performance for large embeddings  
...
Qdrant supports these most popular types of metrics:
  ...
  * Manhattan distance: `Manhattan`*- <https://en.wikipedia.org/wiki/Taxicab_geometry> _*Available as of v1.7_
...
Depending on the query configuration, Qdrant might prefer different strategies for the search. Read more about it in the [query planning](#query-planning) section.
...
Values under the key `params` specify custom parameters for the search. Currently, it could be:
  ...
  * `quantization` \- parameters related to quantization. See [Searching with Quantization](https://qdrant.tech/documentation/manage-data/quantization/#searching-with-quantization) ([local](./../01_manage-data/07_quantization.md#searching-with-quantization)) guide.
  * `acorn` \- parameters related to the [ACORN search algorithm](#acorn-search-algorithm).

Since the `filter` parameter is specified, the search is performed only among those points that satisfy the filter condition. See details of possible filters and their work in the [filtering](https://qdrant.tech/documentation/search/filtering/) ([local](./02_filtering.md)) section.
...
Note that payload and vector data is missing in these results by default. See [payload and vector in the result](#payload-and-vector-in-the-result) on how to include it.
...
For filtered vector search, you are recommended to create a [payload index](https://qdrant.tech/documentation/manage-data/indexing/#payload-index) ([local](./../01_manage-data/06_indexing.md#payload-index)) for the fields you want to filter by. During the search, Qdrant will use a combined [filterable index](https://qdrant.tech/documentation/manage-data/indexing/#filterable-hnsw-index) ([local](./../01_manage-data/06_indexing.md#filterable-hnsw-index)). However, when combining multiple strict payload filters, this mechanism might not provide sufficient accuracy. In such cases, you can use the ACORN search algorithm.

It is an extension to the regular HNSW search algorithm, based on the ACORN-1 algorithm described in the paper [ACORN: Performant and Predicate-Agnostic Search Over Vector Embeddings and Structured Data](https://arxiv.org/abs/2403.04871). During graph traversal, it explores not just direct neighbors (first hop), but also neighbors of neighbors (second hop) when direct neighbors are filtered out. This improves search accuracy at the cost of performance.
...
Whenever you need to use a vector as an input, you can always use a [point ID](https://qdrant.tech/documentation/manage-data/points/#point-ids) ([local](./../01_manage-data/01_points.md#point-ids)) instead.
...
Search and [recommendation](https://qdrant.tech/documentation/search/explore/#recommendation-api) ([local](./04_explore.md#recommendation-api)) APIs allow to skip first results of the search and return only the result starting from some specified offset:
...
For example, if you have a large document split into multiple chunks, and you want to search or [recommend](https://qdrant.tech/documentation/search/explore/#recommendation-api) ([local](./04_explore.md#recommendation-api)) on a per-document basis, you can group the results by the document ID.
...
**Limitations** :
  ...
  * Only [keyword](https://qdrant.tech/documentation/manage-data/payload/#keyword) ([local](./../01_manage-data/03_payload.md#keyword)) and [integer](https://qdrant.tech/documentation/manage-data/payload/#integer) ([local](./../01_manage-data/03_payload.md#integer)) payload values are supported for the `group_by` parameter. Payload values with other types will be ignored.
...
The strategy selection process relies heavily on heuristics and can vary from release to release. However, the general principles are:
  ...
  * planning is performed for each segment independently (see [storage](https://qdrant.tech/documentation/manage-data/storage/) ([local](./../01_manage-data/05_storage.md)) for more information about segments)
  * prefer a full scan if the amount of points is below a threshold
  * estimate the cardinality of a filtered result before selecting a strategy
  * retrieve points using payload index (see [indexing](https://qdrant.tech/documentation/manage-data/indexing/) ([local](./../01_manage-data/06_indexing.md))) if cardinality is below threshold

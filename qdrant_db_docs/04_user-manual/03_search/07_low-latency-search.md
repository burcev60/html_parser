---
title: Tips for Low-Latency Search with Qdrant
...
Qdrant can be deployed in a [distributed configuration](https://qdrant.tech/documentation/distributed_deployment/) ([local](./../../_unsorted/distributed-deployment.md)). In distributed mode, multiple instances of Qdrant, called peers, operate as a single entity, called a cluster. Data is stored in [collections](https://qdrant.tech/documentation/manage-data/collections/) ([local](./../01_manage-data/04_collections.md)), which are divided into [shards](https://qdrant.tech/documentation/distributed_deployment/#sharding) ([local](./../../_unsorted/distributed-deployment.md#sharding)) that are distributed across the peers. Each shard can have multiple [replicas](https://qdrant.tech/documentation/distributed_deployment/#replication) ([local](./../../_unsorted/distributed-deployment.md#replication)) for redundancy and load balancing. Because every replica of the same shard contains the same data, read requests can be distributed across replicas, reducing latency and increasing throughput.
...
![On a cluster with three peers, a collection with 3 shards and a replication factor of 2 would have 6 total replicas distributed across the peers.](https://qdrant.tech/docs/replication.png)
...
Shards store their data in [segments](https://qdrant.tech/documentation/manage-data/storage/) ([local](./../01_manage-data/05_storage.md)). Write operations go through several stages before the changes are fully indexed and searchable:
  ...
  4. Finally, the indexing optimizer creates a [vector index](https://qdrant.tech/documentation/manage-data/indexing/#vector-index) ([local](./../01_manage-data/06_indexing.md#vector-index)) (HNSW graph) for unoptimized segments. Once the indexing process is complete, the data is fully indexed, and search performance is optimal. Indexing is a relatively slow operation, so there can be a delay between when data is written and when it is fully indexed, especially under heavy write load.
...
To restrict searches to indexed data and small segments below the indexing threshold, set the `indexed_only` [search parameter](https://qdrant.tech/documentation/search/search/#search-api) ([local](./01_search.md#search-api)) to `true`. This ensures more consistent and lower response times. However, the tradeoff is that the most recent data might not be included in search results until it has been indexed.
...
### `prevent_unoptimized` Optimizer Setting
 ...
 _Available as of v1.17.1_

`prevent_unoptimized` is an experimental feature; its behavior may change slightly in future releases and it must be used with care.
...
Refer to [Prevent Reads from Large Unindexed Segments](https://qdrant.tech/documentation/ops-optimization/optimizer/#prevent-reads-from-large-unindexed-segments) ([local](./../../_unsorted/ops-optimization/optimizer.md#prevent-reads-from-large-unindexed-segments)) for more details on how this works.

Set the `wait` parameter to `false` on write requests when `prevent_unoptimized` is enabled. See [Effect on `wait=true`](https://qdrant.tech/documentation/ops-optimization/optimizer/#effect-on-waittrue) ([local](./../../_unsorted/ops-optimization/optimizer.md#effect-on-waittrue)).

...
source: https://qdrant.tech/documentation/tutorials-search-engineering/using-relevance-feedback/
---
...
In Qdrant 1.17 we introduced a new [Relevance Feedback Query](https://qdrant.tech/documentation/search/search-relevance/#relevance-feedback) ([local](./../../04_user-manual/03_search/06_search-relevance.md#relevance-feedback)), our scalable, first ever vector index-native approach to [incorporating relevance feedback](https://qdrant.tech/articles/search-feedback-loop/) in retrieval.
...
![Overview of Relevance Feedback](https://qdrant.tech/blog/qdrant-1.17.x/relevance-feedback-overview.png)

### Strategy

To use the feedback for guiding a retriever in the vector space, there are several possible strategies. For now, only the **naive strategy** is available – [a simple 3-parameter formula](https://qdrant.tech/documentation/search/search-relevance/#naive-strategy) ([local](./../../04_user-manual/03_search/06_search-relevance.md#naive-strategy)) which adjusts similarity scoring based on the feedback.
...
We’re going to use a [Qdrant Cloud Free Tier Cluster](https://qdrant.tech/documentation/cloud/create-cluster/#free-clusters) ([local](./../../_unsorted/cloud/create-cluster.md#free-clusters)) (make sure it’s on version 1.17.0+) and [Free Embedding Inference](https://qdrant.tech/documentation/cloud/inference/#free-embedding-models) ([local](./../../_unsorted/cloud/inference.md#free-embedding-models)) available on this Free Tier Cluster.
...
We’ll use one of the datasets available in the Qdrant Cluster UI.
  ...
  2. Out of 3 datasets there, pick the first one, “Qdrant Web Documentation”. It consists of small text chunks from [our documentation website](https://qdrant.tech/documentation/) ([local](./../../index.md)).
  3. Press the `Import` button and type in a collection name – `documentation`.

![Qdrant Cluster UI Datasets](https://qdrant.tech/documentation/tutorials/using-relevance-feedback/dataset.png)

After a couple of seconds you should see a green status pop up _“Snapshot successfully imported”_ and a collection named `documentation` in the list of all collections.
...
![Point in the documentation collection](https://qdrant.tech/documentation/tutorials/using-relevance-feedback/point.png)

Our documentation collection has only one vector per point – `Default vector`. However, in Qdrant, one can have several [named vectors](https://qdrant.tech/documentation/manage-data/vectors/#named-vectors) ([local](./../../04_user-manual/01_manage-data/02_vectors.md#named-vectors)) per point.
...
We also need to point our framework to the raw data which is vectorized with our retriever model. Here, the raw data is the `text` field in the point’s [payload](https://qdrant.tech/documentation/manage-data/payload/#payload) ([local](./../../04_user-manual/01_manage-data/03_payload.md#payload)). Simply put, we search on `text` snippets – in their vectorized form.
...
_If your raw data is stored elsewhere outside of Qdrant, you can redefine the`payload_retrieval` part in `qdrant-relevance-feedback`._
...
As a provider of a feedback model in this tutorial, we’ll use Qdrant’s lightweight inference library called [FastEmbed](https://qdrant.tech/documentation/fastembed/) ([local](./../../08_qdrant-tools/02_fastembed.md)).
...
_This code will download`mxbai-embed-large-v1` for local inference._
...
We’ve generated 50 queries with Claude Code, using the prompt _“Generate 50 short search queries (3-10 words each) that a developer might type when browsing Qdrant’s website.”_.
...
_For training, we need our feedback model to provide ground-truth relevance scores, so it rescores #queries * TRAIN_LIMIT, here 50 * 25 = 1250 query-document pairs. Adjust based on your training budget._
...
_We’re seeing duplicates as our collection consists of chunks from our documentation website and some text is identical across different sections._
...
Now we get feedback from our `mxbai-embed-large-v1` feedback model on the top 3 results for the query _“recommendations API how to use”_.
...
_Approach is tied to what you’re passing to the`example` field in `FeedbackItem` (or analogues in other clients): raw retriever-produced embeddings or point IDs from your collection._
...
We’ll get output looking like this, providing us with _relative results relevance gain_ and _discounted cumulative gain (DCG) win rates_ at _N_ (here, 10).
...
_A detailed explanation of the metrics is in the[article](https://qdrant.tech/articles/search-feedback-loop/), including accompanying visualizations._
...
_If you’d like advice on Relevance Feedback Query usage or have ideas on how to enhance the method, reach out in our[Discord community](https://qdrant.to/discord)._

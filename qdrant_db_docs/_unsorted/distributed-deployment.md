...
source: https://qdrant.tech/documentation/distributed_deployment/
---
...
To enable distributed deployment - enable the cluster mode in the [configuration](https://qdrant.tech/documentation/ops-configuration/configuration/) ([local](./ops-configuration/configuration.md)) or using the ENV variable: `QDRANT__CLUSTER__ENABLED=true`.
...
When you add or remove nodes from the cluster, rebalancing of existing shards across the nodes depends on how you’ve deployed the cluster:
  ...
  * In Qdrant Cloud, shards are [balanced across the nodes automatically](https://qdrant.tech/documentation/cloud/configure-cluster/#shard-rebalancing) ([local](./cloud/configure-cluster.md#shard-rebalancing)).
...
Resharding allows you to change the number of shards in your existing collections if you’re hosting with our [Cloud](https://qdrant.tech/documentation/deploy-intro/) ([local](./../02_deploy.md)) offering.

Resharding can change the number of shards both up and down, without having to recreate the collection from scratch.

Please refer to the [Resharding](https://qdrant.tech/documentation/cloud/cluster-scaling/#resharding) ([local](./cloud/cluster-scaling.md#resharding)) section in our cloud documentation for more details.
...
If your cluster is running in Qdrant Cloud, shards are balanced across the cluster nodes automatically. For more information see the [Configuring Cloud Clusters](https://qdrant.tech/documentation/cloud/configure-cluster/#shard-rebalancing) ([local](./cloud/configure-cluster.md#shard-rebalancing)) and [Cloud Cluster Scaling](https://qdrant.tech/documentation/cloud/cluster-scaling/) ([local](./cloud/cluster-scaling.md)) documentation.
...
You likely want to select a specific [shard transfer method](#shard-transfer-method) to get desired performance and guarantees.

After the transfer is initiated, the service will process it based on the used [transfer method](#shard-transfer-method) keeping both shards in sync. Once the transfer is completed, the old shard is deleted from the source node.
...
For large cardinality keys, it is recommended to use [partition by payload](https://qdrant.tech/documentation/manage-data/multitenancy/#partition-by-payload) ([local](./../04_user-manual/01_manage-data/08_multitenancy.md#partition-by-payload)) instead.
...
Available shard transfer methods are:
  ...
  * `snapshot`: transfer including its index and quantized data by utilizing a [snapshot](https://qdrant.tech/documentation/snapshots/) ([local](./snapshots.md)) automatically.
  * `wal_delta`: _(auto recovery default)_ transfer by resolving [WAL](https://qdrant.tech/documentation/manage-data/storage/#versioning) ([local](./../04_user-manual/01_manage-data/05_storage.md#versioning)) difference; the operations that were missed.
...
**Connectivity**|  Internal gRPC API (6335)| REST API (6333)  
Internal gRPC API (6335)| Internal gRPC API (6335)  
...
**Ordering**|  Unordered updates on target[1](#fn:1)| Ordered updates on target[2](#fn:2)| Ordered updates on target[2](#fn:2)  
...
Use the [Collection Snapshot Recovery API](https://qdrant.tech/documentation/snapshots/#recover-in-cluster-deployment) ([local](./snapshots.md#recover-in-cluster-deployment)) to do it. The service will download the specified snapshot of the collection and recover shards with data from it.
...
![Two clients updating the same point at the same time.](https://qdrant.tech/docs/concurrent-operations-replicas.png)
...
Some [shard transfer methods](#shard-transfer-method) may affect ordering guarantees.
...
Additionally, all write requests sent to the listener node will be processed with `wait=false` option, which means that the write operations will be considered successful once they are written to WAL. This mechanism should allow to minimize upsert latency in case of parallel snapshotting.

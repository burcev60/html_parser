...
source: https://qdrant.tech/documentation/search/hybrid-queries/?q=distribution+based+sc
---
...
With the introduction of [multiple named vectors per point](https://qdrant.tech/documentation/manage-data/vectors/#named-vectors) ([local](./../../../04_user-manual/01_manage-data/02_vectors.md#named-vectors)), there are use-cases when the best search is obtained by combining multiple queries, or by performing the search in more than one stage.
...
Disable the HNSW index for vectors used only for rescoring by setting `m=0` in the vector's HNSW configuration. Rescoring does not use the HNSW index, so disabling it will free up memory.
...
For more information on the `grouping` capabilities refer to the reference documentation for search with [grouping](https://qdrant.tech/documentation/search/search/#search-groups) ([local](./../../../04_user-manual/03_search/01_search.md#search-groups)) and [lookup](https://qdrant.tech/documentation/search/search/#lookup-in-groups) ([local](./../../../04_user-manual/03_search/01_search.md#lookup-in-groups)).

...
source: https://qdrant.tech/documentation/examples/llama-index-multitenancy/
---

# Multitenancy with LlamaIndex

If you are building a service that serves vectors for many independent users, and you want to isolate their data, the best practice is to use a single collection with payload-based partitioning. This approach is called **multitenancy**. Our guide on the [Separate Partitions](https://qdrant.tech/documentation/manage-data/multitenancy/) ([local](./../../04_user-manual/01_manage-data/08_multitenancy.md)) describes how to set it up in general, but if you use [LlamaIndex](https://qdrant.tech/documentation/frameworks/llama-index/) ([local](./../frameworks/llama-index.md)) as a backend, you may prefer reading a more specific instruction. So here it is!
...
_Note_ , in case you are using Large Language Model different from OpenAI’s ChatGPT, you should specify `llm` parameter for `ServiceContext`.
...
The payload index is not the only thing we want to change. Since none of the search queries will be executed on the whole collection, we can also change its configuration, so the HNSW graph is not built globally. This is also done due to [performance reasons](https://qdrant.tech/documentation/manage-data/multitenancy/#calibrate-performance) ([local](./../../04_user-manual/01_manage-data/08_multitenancy.md#calibrate-performance)). **You should not be changing these parameters, if you know there will be some global search operations done on the collection.**

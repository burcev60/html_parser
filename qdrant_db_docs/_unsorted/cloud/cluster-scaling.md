---
title: Scaling Qdrant Cloud Clusters
source: https://qdrant.tech/documentation/cloud/cluster-scaling/
---
...
Important: One shard can not be split across nodes. So, if you configure 2 shards for a collection, but then scale your cluster from 1 to 3 nodes, your cluster nodes can’t be fully utilized. The cloud platform will automatically rebalance your shards, so that two nodes will have one shard each, but the third node will not have any shards at all. You can use the [resharding feature](#resharding) to change the number of shards in an existing collection. Once resharding is complete, the cloud platform will rebalance the shards across all nodes, ensuring that all nodes are utilized.

We will be glad to consult you on an optimal strategy for scaling.

[Let us know](https://qdrant.tech/documentation/support/) ([local](./../support.md)) your needs and decide together on a proper solution.

## Resharding

 _Available as of Qdrant v1.13.0_

Resharding is exclusively available on multi-node clusters across our [cloud](https://qdrant.tech/documentation/deploy-intro/) ([local](./../../02_deploy.md)) offering, including [Hybrid](https://qdrant.tech/documentation/hybrid-cloud/) ([local](./../hybrid-cloud.md)) and [Private](https://qdrant.tech/documentation/private-cloud/) ([local](./../private-cloud.md)) Cloud.

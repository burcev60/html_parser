---
title: Backing up Qdrant Cloud Clusters
source: https://qdrant.tech/documentation/cloud/backups/
---
...
You can back up your Qdrant clusters though the Qdrant Cloud Dashboard at <https://cloud.qdrant.io>. This section assumes that you’ve already set up your cluster, as described in the following sections:
  ...
  * [Create a cluster](https://qdrant.tech/documentation/cloud/create-cluster/) ([local](./create-cluster.md))
  * Set up [Authentication](https://qdrant.tech/documentation/cloud/authentication/) ([local](./authentication.md))
  * Configure one or more [Collections](https://qdrant.tech/documentation/manage-data/collections/) ([local](./../../04_user-manual/01_manage-data/04_collections.md))
...
Qdrant also offers a snapshot API which allows you to create a snapshot of a specific collection or your entire cluster. For more information, see our [snapshot documentation](https://qdrant.tech/documentation/snapshots/) ([local](./../snapshots.md)).
...
Here is how you can take a snapshot and recover a collection:
  ...
  1. Take a snapshot:
     ...
     * For a multi node cluster call a snapshot on each node of the collection. Specifically, prepend `node-{num}-` to your cluster URL. Then call the [snapshot endpoint](https://qdrant.tech/documentation/snapshots/#create-snapshot) ([local](./../snapshots.md#create-snapshot)) on the individual hosts. Start with node 0.
     * In the response, you’ll see the name of the snapshot.
  2. Delete and recreate the collection.
  3. Recover the snapshot:
     * Call the [recover endpoint](https://qdrant.tech/documentation/snapshots/#recover-in-cluster-deployment) ([local](./../snapshots.md#recover-in-cluster-deployment)). Set a location which points to the snapshot file (`file:///qdrant/snapshots/{collection_name}/{snapshot_file_name}`) for each host.

...
source: https://qdrant.tech/documentation/migration-guidance/
---
...
Before starting verification, you need:
  ...
  * The [Qdrant Migration Tool](https://qdrant.tech/documentation/migrate-to-qdrant/) ([local](./migrate-to-qdrant.md)) already run
...
This guide proposes four distinct stages of verification, each building on the next to ensure a successful migration:
  ...
  1. **[Pre-Migration Baseline](https://qdrant.tech/documentation/migration-guidance/pre-migration-baseline/) ([local](./migration-guidance/pre-migration-baseline.md)):** Before starting the migration, capture what “correct” looks like in your source system. This includes vector counts, metadata samples, collection configuration, and baseline search results. Without this, post-migration comparison is impossible. _Budget 15 to 30 minutes_.
  2. **[Data Integrity](https://qdrant.tech/documentation/migration-guidance/data-integrity/) ([local](./migration-guidance/data-integrity.md)):** After migration, verify that the data arrived intact. This layer catches missing vectors, dropped metadata fields, type coercion errors, and misconfigured collections. These checks take only minutes to run and catch the most common migration failures.
  3. **[Search Quality](https://qdrant.tech/documentation/migration-guidance/search-quality/) ([local](./migration-guidance/search-quality.md)):** This layer catches result ranking changes, recall degradation, and relevance shifts caused by differences in indexing, quantization, or scoring between systems. _Depending on the tier you choose, this takes anywhere from 15 minutes to several hours._
  4. **[Diagnosing Discrepancies](https://qdrant.tech/documentation/migration-guidance/diagnosing-discrepancies/) ([local](./migration-guidance/diagnosing-discrepancies.md)):** When any of the previous layers catches a problem, this section provides a decision tree for root-causing it: is the issue in the data or the configuration? Which vendor-specific gotcha is responsible? What’s the fastest path to resolution?
...
**Next:** [Pre-Migration Baseline](https://qdrant.tech/documentation/migration-guidance/pre-migration-baseline/) ([local](./migration-guidance/pre-migration-baseline.md))

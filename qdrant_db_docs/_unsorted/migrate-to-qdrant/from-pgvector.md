---
title: Migrate from pgvector to Qdrant
source: https://qdrant.tech/documentation/migrate-to-qdrant/from-pgvector/
---
...
## Gotchas
  ...
  * **Row count approximation:** Postgres’s `n_live_tup` is an estimate, not an exact count. Use `SELECT COUNT(*) FROM your_table` for accurate comparison during [migration verification](https://qdrant.tech/documentation/migration-guidance/) ([local](./../migration-guidance.md)).

## After Migration: Keeping Postgres and Qdrant in Sync

If you continue using Postgres as your source of truth alongside Qdrant, you’ll need a sync strategy. The [Data Synchronization Guide](https://qdrant.tech/documentation/data-synchronization/) ([local](./../data-synchronization.md)) covers three approaches from simple dual-writes to production-grade Change Data Capture.

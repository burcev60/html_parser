---
title: Frequently Asked Questions: General Topics
source: https://qdrant.tech/documentation/faq/qdrant-fundamentals/
---
...
| | | |   
---|---|---|---|---  
[Vectors](#vectors)| [Search](#search)| [Collections](#collections)| [Compatibility](#compatibility)| [Cloud](#cloud)  
...
There is no inherent limitation on metadata size, but it should be [optimized for performance and resource usage](https://qdrant.tech/documentation/ops-optimization/optimize/) ([local](./../_unsorted/ops-optimization/optimize.md)). Users can set upper limits in the configuration.
...
Qdrant natively [supports multiple vectors per data point](https://qdrant.tech/documentation/manage-data/vectors/#multivectors) ([local](./../04_user-manual/01_manage-data/02_vectors.md#multivectors)), allowing different embeddings from various providers to coexist within the same collection.
...
Collection info API in Qdrant returns an approximate number of points in the collection. If you need an exact number, you can use the [count](https://qdrant.tech/documentation/manage-data/points/#counting-points) ([local](./../04_user-manual/01_manage-data/01_points.md#counting-points)) API.

### Vectors in the collection don’t match what I uploaded.

There are two possible reasons for this:

  * You used the `Cosine` distance metric in the [collection settings](https://qdrant.tech/documentation/manage-data/collections/#collections) ([local](./../04_user-manual/01_manage-data/04_collections.md#collections)). In this case, Qdrant pre-normalizes your vectors for faster distance computation. If you strictly need the original vectors to be preserved, consider using the `Dot` distance metric instead.
  * You used the `uint8` [datatype](https://qdrant.tech/documentation/manage-data/vectors/#datatypes) ([local](./../04_user-manual/01_manage-data/02_vectors.md#datatypes)) to store vectors. `uint8` requires a special format for input values, which might not be compatible with the typical output of embedding models.

### How many vectors can I store in a point? Can a point have no vector at all?

A point can hold any number of dense, sparse, and multi vectors, though each has to be configured in the collection’s schema. There’s no hard limit imposed by Qdrant, though practical limits apply: each additional vector increases memory usage, so the realistic ceiling is determined by available RAM and storage. You can attach a single vector, or multiple vectors with different names (for example, a dense vector for semantic search alongside a sparse vector for keyword matching). This lets you run [hybrid queries](https://qdrant.tech/documentation/search/hybrid-queries/) ([local](./../04_user-manual/03_search/03_hybrid-queries.md)) over several representations of the same data within one collection. Each vector must be defined in the collection’s schema.

A point can also have zero vectors. If you don’t provide any vectors at upsert time, Qdrant stores the point with its ID and payload only. This is useful when you want to use Qdrant as a document store with filtering, or when you plan to add vectors to a point later. A vector-less point won’t appear in nearest-neighbor search results, but it’s fully accessible via [scroll](https://qdrant.tech/documentation/manage-data/points/#scroll-points) ([local](./../04_user-manual/01_manage-data/01_points.md#scroll-points)) and payload filtering.

### Can Qdrant generate vector embeddings?

Yes, if you’re using Qdrant Cloud, you can generate embeddings with [**Qdrant Cloud Inference**](https://qdrant.tech/documentation/inference/) ([local](./../05_inference.md)). It lets you embed, store, and index your data in a single API call, so you don’t need a separate inference service or embedding pipeline.
...
If you’re running Qdrant open-source or self-hosted, Cloud Inference isn’t available. You can use [FastEmbed](https://qdrant.tech/documentation/fastembed/) ([local](./../08_qdrant-tools/02_fastembed.md)), Qdrant’s lightweight, local inference library, or bring your own embedding model or service. See the [Embeddings documentation](https://qdrant.tech/documentation/embeddings/) ([local](./../_unsorted/embeddings.md)) for supported models and providers.
...
See also: [Text Chunking Strategies](https://qdrant.tech/documentation/faq/qdrant-fundamentals/course/essentials/day-1/chunking-strategies/)
...
You are likely looking for the [scroll](https://qdrant.tech/documentation/manage-data/points/#scroll-points) ([local](./../04_user-manual/01_manage-data/01_points.md#scroll-points)) method. It allows you to retrieve the records based on filters or even iterate over all the records in the collection.

### My filtered vector search is slow. What should I check first?

Add a [payload index](https://qdrant.tech/documentation/manage-data/indexing/#payload-index) ([local](./../04_user-manual/01_manage-data/06_indexing.md#payload-index)) on all the fields you’re filtering by. Payload indexing often produces larger speedups for filtered queries than other optimizations such as changes to Hierarchical Navigable Small World (HNSW) parameters.
...
To prevent clients from filtering on payload fields that don’t have a payload index, enable strict mode and [set unindexed_filtering_retrieve to false](https://qdrant.tech/documentation/ops-configuration/administration/#disable-retrieving-via-non-indexed-payload) ([local](./../_unsorted/ops-configuration/administration.md#disable-retrieving-via-non-indexed-payload)).

See also: [Indexing](https://qdrant.tech/documentation/manage-data/indexing/) ([local](./../04_user-manual/01_manage-data/06_indexing.md)), [Low-Latency Search](https://qdrant.tech/documentation/search/low-latency-search/) ([local](./../04_user-manual/03_search/07_low-latency-search.md))

### Does Qdrant support a full-text search or a hybrid search?

Qdrant is a vector search engine in the first place, and we only implement full-text support as long as it doesn’t compromise the vector search use case. That includes both the interface and the performance.
...
What Qdrant can do:
  ...
  * [Multi-vectors](https://qdrant.tech/documentation/manage-data/vectors/#multivectors) ([local](./../04_user-manual/01_manage-data/02_vectors.md#multivectors)), for example ColBERT and other late-interaction models
  * Combination of the [multiple searches](https://qdrant.tech/documentation/search/hybrid-queries/) ([local](./../04_user-manual/03_search/03_hybrid-queries.md))
...
Of course, you can always combine Qdrant with any specialized tool you need, including full-text search engines. Read more about [our approach](https://qdrant.tech/articles/hybrid-search/) to hybrid search.
...
Both methods combine scores from multiple retrieval legs (for example, dense and sparse), but they work differently:
  ...
  * **RRF (Reciprocal Rank Fusion)** combines ranked lists based on position, not score magnitude. It works well when scores from different retrieval methods are on incompatible scales (common with dense vs. sparse). Start here by default. [Tune weights and k](https://qdrant.tech/documentation/search/hybrid-queries/#setting-rrf-constant-k) ([local](./../04_user-manual/03_search/03_hybrid-queries.md#setting-rrf-constant-k)) as needed.
...
For custom fusion, use the [Formula Query](https://qdrant.tech/documentation/search/search-relevance/#score-boosting) ([local](./../04_user-manual/03_search/06_search-relevance.md#score-boosting)). For example, you can use decay functions to normalize both scores to a 0-1 range and then fuse them. This approach requires you to determine the approximate score distribution for each corpus, since you can’t set decay function parameters dynamically. The Formula Query doesn’t support custom rank-based fusion because it doesn’t have access to prefetch ranks; only to the raw scores.

To evaluate which works better for your use case, create a small golden query set and compare [retrieval quality metrics](https://qdrant.tech/documentation/improve-search/retrieval-relevance/) ([local](./../_unsorted/improve-search/retrieval-relevance.md)) (for example, NDCG@10) under each method.

See also: [Hybrid Queries](https://qdrant.tech/documentation/search/hybrid-queries/) ([local](./../04_user-manual/03_search/03_hybrid-queries.md))

### My hybrid search results aren’t relevant. Where do I start debugging?

Work through these in order:

  1. **Check sparse preprocessing.** Poor sparse results are often a tokenization issue. For non-English text, configure language-specific stemming and stop-word lists in the [text search settings](https://qdrant.tech/documentation/search/text-search/#language-specific-settings) ([local](./../04_user-manual/03_search/05_text-search.md#language-specific-settings)).
  ...
  4. **Add a reranking stage.** If precision matters more than latency, [reranking](https://qdrant.tech/documentation/search/hybrid-queries/#multi-stage-queries) ([local](./../04_user-manual/03_search/03_hybrid-queries.md#multi-stage-queries)) as a final stage can recover from imperfect retrieval.
...
There are three strategies:
  ...
  3. **Custom sharding (`shard_key_selector`)** — hard physical boundaries with distinct shards. Eliminates noisy-neighbor problems. Recommended only for low cardinality (< ~1,000 unique values) and only when you _always_ filter on that field.
...
As many as you want, but be aware that each collection requires additional resources. It is _highly_ recommended not to create many small collections, as it will lead to significant resource consumption overhead.

We consider creating a collection for each user/dialog/document as an antipattern.

Read more about collections, isolation, and multiple users in our [Multitenancy](https://qdrant.tech/documentation/manage-data/multitenancy/) ([local](./../04_user-manual/01_manage-data/08_multitenancy.md)) documentation.
...
See also: [Named Vectors](https://qdrant.tech/documentation/manage-data/vectors/#named-vectors) ([local](./../04_user-manual/01_manage-data/02_vectors.md#named-vectors))
...
The recommended pattern is an alias-based swap:
  ...
  2. Once indexed, atomically update the [collection alias](https://qdrant.tech/documentation/manage-data/collections/#collection-aliases) ([local](./../04_user-manual/01_manage-data/04_collections.md#collection-aliases)) to point to the new collection.
...
Common recovery steps:
  ...
  2. Send any [update collection operation](https://qdrant.tech/documentation/manage-data/collections/#grey-collection-status) ([local](./../04_user-manual/01_manage-data/04_collections.md#grey-collection-status)) to trigger and start the optimizations again.

See also: [Grey collection status](https://qdrant.tech/documentation/manage-data/collections/#grey-collection-status) ([local](./../04_user-manual/01_manage-data/04_collections.md#grey-collection-status))

### How do I upload a large number of vectors into a Qdrant collection?

Read about our recommendations in the [bulk upload](https://qdrant.tech/documentation/tutorials-develop/bulk-upload/) ([local](./../09_tutorials/07_develop-implement/01_bulk-operations.md)) tutorial.
...
A useful pattern for large-scale bulk loads is **staged indexing** : disable HNSW graph construction during upload [by setting the HNSQ `m` parameter to `0`](https://qdrant.tech/documentation/tutorials-develop/bulk-upload/#defer-hnsw-graph-construction-m-0) ([local](./../09_tutorials/07_develop-implement/01_bulk-operations.md#defer-hnsw-graph-construction-m-0)), upload in batches using the upsert API, then restore the threshold to trigger background indexing once the load is complete. This avoids optimizer thrashing and significantly improves throughput during the initial load.

See also: [Bulk Operations](https://qdrant.tech/documentation/tutorials-develop/bulk-upload/) ([local](./../09_tutorials/07_develop-implement/01_bulk-operations.md))
...
Instead of creating separate collections for each user, we recommend creating a [single collection](https://qdrant.tech/documentation/manage-data/multitenancy/) ([local](./../04_user-manual/01_manage-data/08_multitenancy.md)) and separate access using payloads. Each Qdrant point can have a payload as metadata. For multitenancy, you can include a `user_id` or `tenant_id` for each point. To optimize storage further, you can enable [tenant indexing](https://qdrant.tech/documentation/manage-data/indexing/#tenant-index) ([local](./../04_user-manual/01_manage-data/06_indexing.md#tenant-index)) for payload fields.

## Cloud

### Is it possible to scale down a Qdrant Cloud cluster?

Yes, it is possible to both vertically and horizontally scale down a Qdrant Cloud cluster. Note, that during the vertical scaling down, the disk size cannot be reduced.

---
title: Inference
source: https://qdrant.tech/documentation/inference/
---
...
![Inference](https://qdrant.tech/docs/inference.png)
...
Depending on the model you want to use, inference can be executed:
  ...
  * on the client side, using the [FastEmbed](https://qdrant.tech/documentation/fastembed/) ([local](./08_qdrant-tools/02_fastembed.md)) library
...
You can use inference in the API wherever you can use regular vectors. Instead of a vector, you can use special _Inference Objects_ :
...
Clusters on Qdrant Managed Cloud can access embedding models that are [hosted on Qdrant Cloud](https://qdrant.tech/documentation/cloud/inference/) ([local](./_unsorted/cloud/inference.md)). For a list of available models, visit the Inference tab of the Cluster Detail page in the Qdrant Cloud Console. Here, you can also enable Cloud Inference for a cluster if it’s not already enabled.
...
The Python SDK offers a unique capability: it supports both [local](https://qdrant.tech/documentation/fastembed/fastembed-semantic-search/) ([local](./08_qdrant-tools/01_fastembed/02_fastembed-qdrant.md)) and cloud inference through an identical interface.
...
![Inference with an external embedding model provider](https://qdrant.tech/docs/inference-external-provider.png)
...
A good use case for MRL is [prefetching](https://qdrant.tech/documentation/search/hybrid-queries/#multi-stage-queries) ([local](./04_user-manual/03_search/03_hybrid-queries.md#multi-stage-queries)) with smaller vectors, followed by re-scoring with the original-sized vectors, effectively balancing speed and accuracy. This example first prefetches 1000 candidates using a 64-dimensional reduced vector (`small`) and then re-scores them using the original full-size vector (`large`) to return the top 10 most relevant results:

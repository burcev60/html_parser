...
Without any special configuration, mypy does not catch the [missing model field annotation](https://pydantic.dev/docs/validation/latest/errors/usage_errors#model-field-missing-annotation) ([local](./../../errors/usage-errors.md#model-field-missing-annotation)) and errors about the `list_of_ints` argument which Pydantic parses correctly:
...
To enable the plugin, just add `pydantic.mypy` to the list of plugins in your [mypy config file](https://mypy.readthedocs.io/en/latest/config_file.html):
  ...
  * [ mypy.ini ](#tab-panel-703)
  * [ pyproject.toml ](#tab-panel-704)
...
#respect-the-type-of-the-fields-default-and-default_factory
...
#warn-about-the-use-of-untyped-fields
  ...
  * While defining a field without an annotation will result in a [runtime error](https://pydantic.dev/docs/validation/latest/errors/usage_errors#model-field-missing-annotation) ([local](./../../errors/usage-errors.md#model-field-missing-annotation)), the plugin will also emit a type checking error.
...
A configuration file with all plugin strictness flags enabled (and some other mypy strictness flags, too) might look like:
  ...
  * [ mypy.ini ](#tab-panel-705)
  * [ pyproject.toml ](#tab-panel-706)
...
For this reason, the plugin will use `Any` for field annotations when synthesizing the `__init__` method, unless `init_typed` is set or [strict mode](https://pydantic.dev/docs/validation/latest/concepts/strict_mode) ([local](./../../concepts/strict-mode.md)) is enabled on the model.

...
source: https://fastapi.tiangolo.com/ru/deployment/docker/
---
...
[Английская версия](https://fastapi.tiangolo.com/deployment/docker/)
...
Разумеется, вы будете придерживаться тех же идей, что описаны здесь: [О версиях FastAPI](https://fastapi.tiangolo.com/ru/deployment/versions/) ([local](./01_о-версиях-fastapi.md)), чтобы задать диапазоны версий.
...
Обязательно используйте **exec** -форму, чтобы FastAPI мог корректно завершаться и чтобы срабатывали [события lifespan](https://fastapi.tiangolo.com/ru/advanced/events/) ([local](./../08_расширенное-руководство-пользователя/20_события-lifespan.md)).
...
Ещё раз рассмотрим [концепции развертывания](https://fastapi.tiangolo.com/ru/deployment/concepts/) ([local](./05_концепции-развёртывания.md)) применительно к контейнерам.
...
Обычно такой **балансировщик нагрузки** может также обрабатывать запросы к _другим_ приложениям в вашем кластере (например, к другому домену или под другим префиксом пути URL) и направлять их к нужным контейнерам этого _другого_ приложения.

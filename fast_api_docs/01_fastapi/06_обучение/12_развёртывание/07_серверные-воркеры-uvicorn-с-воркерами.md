...
source: https://fastapi.tiangolo.com/ru/deployment/server-workers/
---
...
[Английская версия](https://fastapi.tiangolo.com/deployment/server-workers/)
...
Как вы видели в предыдущей главе о [Концепциях деплоя](https://fastapi.tiangolo.com/ru/deployment/concepts/) ([local](./05_концепции-развёртывания.md)), существует несколько стратегий.

Здесь я покажу, как использовать **Uvicorn** с **воркер-процессами** через команду `fastapi` или напрямую через команду `uvicorn`.

Информация

Если вы используете контейнеры, например Docker или Kubernetes, я расскажу об этом подробнее в следующей главе: [FastAPI в контейнерах — Docker](https://fastapi.tiangolo.com/ru/deployment/docker/) ([local](./08_fastapi-в-контейнерах-docker.md)).
...
В следующей главе о [FastAPI в контейнерах — Docker](https://fastapi.tiangolo.com/ru/deployment/docker/) ([local](./08_fastapi-в-контейнерах-docker.md)) я объясню стратегии, которые можно использовать для решения остальных **концепций деплоя**.

...
source: https://fastapi.tiangolo.com/ru/tutorial/security/first-steps/
---
...
[Английская версия](https://fastapi.tiangolo.com/tutorial/security/first-steps/)
...
Чтобы установить его вручную, убедитесь, что вы создали [виртуальное окружение](https://fastapi.tiangolo.com/ru/virtual-environments/) ([local](./../../04_виртуальные-окружения.md)), активировали его и затем установили пакет:
...
А у вашей _операции пути_ в правом верхнем углу есть маленький замочек, на который можно нажать.
...
Использование относительного URL важно для того, чтобы ваше приложение продолжало работать даже в таком продвинутом случае, как [За прокси-сервером](https://fastapi.tiangolo.com/ru/advanced/behind-a-proxy/) ([local](./../../08_расширенное-руководство-пользователя/17_за-прокси-сервером.md)).

Этот параметр не создает конечную точку / _операцию пути_ , а объявляет, что URL `/token` — это тот, который клиент должен использовать для получения токена. Эта информация используется в OpenAPI, а затем в интерактивных системах документации по API.
...
Эта зависимость предоставит `str`, который будет присвоен параметру `token` _функции-обработчика пути_.

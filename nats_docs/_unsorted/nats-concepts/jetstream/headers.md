...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/nats-concepts/jetstream/headers.md)chevron-down
...
[hashtag](#republish-or-direct-get)
...
Number of messages pending in the multi/batched get response. or details see: [ADR-31arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-31.md)
...
On the last messages of multi/batched get response. The `up-to-seq` value of the original request. Helps the client to continue incomplete batch requests. For details see: [ADR-31arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-31.md)
...
Introduced in version 2.11 - see [ADR-41arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-41.md)
...
Triggers tracing as per the <https://www.w3.org/TR/trace-context/>[arrow-up-right](https://www.w3.org/TR/trace-context/) standard. Requires tracing destination and sampling rate to be configured in the `msg_trace` section on the account level. See [Tracing open telemetry style](https://docs.nats.io/running-a-nats-service/configuration) ([local](./../../../09_chevron-right/08_configuring-nats-server-chevron-right.md))
...
Introduced in version 2.12 with extensions in 2.14 - see [ADR-51arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-51.md)
...
`0 30 14 * * *`, `@hourly`, `@daily`, `@every 5m`, `@at 2009-11-10T23:00:00Z` (RFC3339 format)

2.12 (repeating schedules: 2.14)
...
Optional. The rollup to be set on the final message on the target subject.
...
Optional. Instructs the schedule to read the last message on the given subject and publish it. If the Subject is empty, the schedule message's content is the same as the schedule's content. Wildcards are not supported.
...
The rollup value when `Nats-Schedule-Rollup` was set
...
[hashtag](#atomic-batch-publish)

Atomic batch publish

Introduced in version 2.12 with optimizations in 2.14 - see [ADR-50arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-50.md)
...
When messages are removed from a KV where subject delete markers are supported, a delete marker will be placed. And notifications are sent to interested watchers. The message payload is empty and the removal reason is indicated through this header. See [ADR-48arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-48.md)
...
Used in KV stores to atomically increment counter. Any valid integer (including 0) starting with a sign.. See [ADR-49arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-49.md)
...
Tracking `Nats-Incr` when messages are sourced. For details see: [ADR-49arrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-49.md)
...
[PreviousObject Store Walkthroughchevron-left](https://docs.nats.io/nats-concepts/jetstream/obj_store/obj_walkthrough) ([local](./obj-store/obj-walkthrough.md))[NextSubject Mapping and Partitioningchevron-right](https://docs.nats.io/nats-concepts/subject_mapping) ([local](./../../../05_chevron-right/10_subject-mapping-and-partitioning.md))

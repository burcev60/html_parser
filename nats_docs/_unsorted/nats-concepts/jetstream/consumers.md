...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/nats-concepts/jetstream/consumers.md)chevron-down
...
Unlike [Core NATSarrow-up-right](https://docs.nats.io/nats-concepts/core-nats) ([local](./../../../05_chevron-right/06_core-nats-chevron-right.md)), which provides an at most once delivery guarantee, a consumer in JetStream can provide an at least **once delivery** guarantee.
...
[hashtag](#dispatch-type-pull-push)
...
[hashtag](#persistence-durable-ephemeral)
...
If set, clients can have subscriptions bind to the consumer and _resume_ until the consumer is explicitly deleted. A durable name cannot contain whitespace, `.`, `*`, `>`, path separators (forward or backward slash), or non-printable characters.
...
The requirement of client acknowledgments, either `AckExplicit`, `AckNone`, `AckAll`, or `AckFlowControl` (2.14).
...
The duration that the server will wait for an acknowledgment for any individual message _once it has been delivered to a consumer_. If an acknowledgment is not received in time, the message will be redelivered. This setting is only effective when `BackOff` is **not configured**.
...
Defines the maximum number of messages, without acknowledgment, that can be outstanding. Once this limit is reached, message delivery will be suspended. This limit applies across _all_ of the consumer's bound subscriptions. A value of -1 means there can be any number of pending acknowledgments (i.e., no flow control). The default is 1000.
...
[hashtag](#pull-specific)

Pull-specific

These options apply only to pull consumers. For configuration examples, see [NATS by Examplearrow-up-right](https://natsbyexample.com/examples/jetstream/pull-consumer/go).
...
[hashtag](#push-specific)

Push-specific

These options apply only to push consumers. For configuration examples, see [NATS by Examplearrow-up-right](https://natsbyexample.com/examples/jetstream/push-consumer/go).
...
The subject to deliver messages to. Setting this field decides whether the consumer is push or pull-based. With a deliver subject, the server will _push_ messages to clients subscribed to this subject.
...
The queue group name used to distribute messages among subscribers. Analogous to a [queue grouparrow-up-right](https://docs.nats.io/nats-concepts/core-nats/queue) ([local](./../core-nats/queue.md)) in core NATS.
...
[PreviousExamplechevron-left](https://docs.nats.io/nats-concepts/jetstream/source_and_mirror/source_and_mirror_example) ([local](./source-and-mirror/source-and-mirror-example.md))[NextExamplechevron-right](https://docs.nats.io/nats-concepts/jetstream/consumers/example_configuration) ([local](./consumers/example-configuration.md))

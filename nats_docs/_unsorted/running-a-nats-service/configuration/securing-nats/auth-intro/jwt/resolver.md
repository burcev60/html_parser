---
title: Account lookup using Resolver
source: https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt/resolver
---

[githubEdit](https://github.com/nats-io/nats.docs/blob/master/running-a-nats-service/configuration/securing_nats/jwt/resolver.md)chevron-down
...
block-quoteOn this pageblock-quote
  ...
  5. [Decentralized JWT Authentication/Authorization](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt) ([local](./../../../../../../14_hashtag-guided-tour/06_zero-trust-security.md))

# Account lookup using Resolver

The `resolver` configuration option is used in conjunction with [NATS JWT Authentication](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt) ([local](./../../../../../../14_hashtag-guided-tour/06_zero-trust-security.md)) and [nsc](https://docs.nats.io/using-nats/nats-tools/nsc) ([local](./../../../../../using-nats/nats-tools/nsc.md)). The `resolver` option specifies a URL where the nats-server can retrieve an account JWT. There are 3 resolver implementations:

  * [NATS Based Resolver](#nats-based-resolver) which is the preferred option and should be your default selection

  * [`MEMORY`](#memory) if you want to statically define the accounts in the server configuration

  * [`URL`](#url-Resolver) if you want to build your own account service, typically in order to have some integration of NATS security with some external security system.
...
In this mode of operation administrators typically use the [`nsc`](https://docs.nats.io/using-nats/nats-tools/nsc) ([local](./../../../../../using-nats/nats-tools/nsc.md)) CLI tool to create/manage the JWTs locally, and use `nsc push` to push new JWTs to the nats-servers' built-in resolvers, `nsc pull` to refresh their local copy of account JWTs, and `nsc revocations` to revoke them.
...
[hashtag](#nats-based-resolver-integration)
...
To upload or update any generated account JWT without [`nsc`](https://docs.nats.io/using-nats/nats-tools/nsc) ([local](./../../../../../using-nats/nats-tools/nsc.md)), send it as a request to `$SYS.REQ.CLAIMS.UPDATE`. Each participating `full` NATS-based account resolver will respond with a message detailing success or failure.
...
[hashtag](#migrating-account-data)
...
For more information on how to configure a memory resolver, see [this tutorial](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt/mem_resolver) ([local](./mem-resolver.md)).
...
**NOTE:** The [standalone NATS Account JWT Serverarrow-up-right](https://nats-io.gitbook.io/legacy-nats-docs/nats-account-server) is now _legacy_ , please use the [NATS Based Resolver](#nats-based-resolver) instead. However, the URL resolver option is still available in case you want to implement your own version of an account resolver
...
If the server used requires client authentication, or you want to specify which CA is trusted for the lookup of account information, specify `resolver_tls`. This [`tls` configuration map](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/tls) ([local](./../../tls.md)) lets you further restrict TLS to the resolver.

[PreviousDecentralized JWT Authentication/Authorizationchevron-left](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt) ([local](./../../../../../../14_hashtag-guided-tour/06_zero-trust-security.md))[NextMemory Resolver Tutorialchevron-right](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt/mem_resolver) ([local](./mem-resolver.md))

---
title: Configuration
source: https://docs.nats.io/running-a-nats-service/configuration/leafnodes/leafnode_conf
---

[githubEdit](https://github.com/nats-io/nats.docs/blob/master/running-a-nats-service/configuration/leafnodes/leafnode_conf.md)chevron-down
...
block-quoteOn this pageblock-quote
  ...
  3. [Leaf Nodes](https://docs.nats.io/running-a-nats-service/configuration/leafnodes) ([local](./../leafnodes.md))

# Configuration

## 

[hashtag](#leafnodes-configuration-block)
...
TLS configuration block (same as other nats-server [`tls` configuration](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/tls) ([local](./../securing-nats/tls.md))).
...
Authorization block. [**See Authorization Block section below**](#authorization-block).

`remotes`

List of [`remote`](#leafnode-remotes-entry-block) entries specifying servers where leafnode client connection can be made.
...
Configures compression of leafnode connections similar to [cluster routes](https://docs.nats.io/running-a-nats-service/configuration/clustering/v2_routes) ([local](./../clustering/v2-routes.md)). Defaults to `s2_auto`. See details [here](https://docs.nats.io/running-a-nats-service/configuration/clustering/v2_routes#compression) ([local](./../clustering/v2-routes.md#compression)).
...
[Account](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/accounts) ([local](./../securing-nats/accounts.md)) this leaf node connection should be bound to.
...
List of credentials and account to bind to leaf node connections. [**See User Block section below**](#users-block).
...
[hashtag](#leafnode-remotes-entry-block)
...
[Account](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/accounts) ([local](./../securing-nats/accounts.md)) name or JWT public key identifying the local account to bind to this remote server. Any traffic locally on this account will be forwarded to the remote server.
...
A [TLS configuration](#tls-configuration-block) block. Leafnode client will use specified TLS certificates when connecting/authenticating.

`ws_compression`

If connecting with [Websocket](#connecting-using-websocket-protocol) protocol, this boolean (`true` or `false`) indicates to the remote server that it wishes to use compression. The default is `false`.

`ws_no_masking`

If connecting with [Websocket](#connecting-using-websocket-protocol) protocol, this boolean indicates to the remote server that it wishes not to mask outbound WebSocket frames. The default is `false`, which means that outbound frames will be masked.
...
And example of how to use it can be found [herearrow-up-right](https://github.com/nats-io/nats-server/blob/7baf7bd8870a0719e3692e6523b09a14653f717d/server/leafnode_test.go#L4402)

### 

[hashtag](#connecting-using-websocket-protocol)
...
To configure Websocket in the remote server, check the [Websocket](https://docs.nats.io/running-a-nats-service/configuration/websocket) ([local](./../websocket.md)) section.

### 

[hashtag](#tls-configuration-block)
...
[PreviousLeaf Nodeschevron-left](https://docs.nats.io/running-a-nats-service/configuration/leafnodes) ([local](./../leafnodes.md))[NextJetStream on Leaf Nodeschevron-right](https://docs.nats.io/running-a-nats-service/configuration/leafnodes/jetstream_leafnodes) ([local](./jetstream-leafnodes.md))

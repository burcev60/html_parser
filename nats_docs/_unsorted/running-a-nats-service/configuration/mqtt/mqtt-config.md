...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/running-a-nats-service/configuration/mqtt/mqtt_config.md)chevron-down
...
block-quoteOn this pageblock-quote
  ...
  3. [MQTT](https://docs.nats.io/running-a-nats-service/configuration/mqtt) ([local](./../mqtt.md))
...
[hashtag](#authentication-authorization-of-mqtt-users)
...
In operator mode, all users need to provide a JWT in order to connect. In the standard authentication procedure of this mode, NATS clients are required to sign a `nonce` sent by the server using their private key (see [JWTs and Privacy](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/jwt#jwts-and-privacy) ([local](./../../../../14_hashtag-guided-tour/06_zero-trust-security.md#jwts-and-privacy))). MQTT clients cannot do that, therefore, the JWT is used for authentication, removing the need of the seed. It means that you need to pass the JWT token as the MQTT password and use any username (except empty, since MQTT protocol requires a username to be set if a password is set). The JWT has to have the `Bearer` boolean set to true, which can be done with nsc:
...
[PreviousMQTTchevron-left](https://docs.nats.io/running-a-nats-service/configuration/mqtt) ([local](./../mqtt.md))[NextConfiguring Subject Mappingchevron-right](https://docs.nats.io/running-a-nats-service/configuration/configuring_subject_mapping) ([local](./../configuring-subject-mapping.md))

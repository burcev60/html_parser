...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/running-a-nats-service/configuration/gateways/gateway.md)chevron-down
...
block-quoteOn this pageblock-quote
  ...
  3. [Super-cluster with Gateways](https://docs.nats.io/running-a-nats-service/configuration/gateways) ([local](./../gateways.md))

# Configuration

The `gateway` [configuration block](#gateway-configuration-block) is similar to a [`cluster`](https://docs.nats.io/running-a-nats-service/configuration/clustering/cluster_config) ([local](./../clustering/cluster-config.md)) block:
...
[hashtag](#gateway-configuration-block)
...
List of Gateway [entries](#gateway-entry) \- see below.
...
A [`tls` configuration map](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/tls) ([local](./../securing-nats/tls.md)) for securing gateway connections. `verify` is always enabled. Unless otherwise specified in a [`gateway`](#gateway-entry), `cert_file` will be the default client certificate. [Certificate pitfalls](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/tls) ([local](./../securing-nats/tls.md)).
...
A [`tls` configuration map](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/tls) ([local](./../securing-nats/tls.md)) for creating a secure gateway connection. If the top-level `gateway{}` tls block contains certificates that have both client and server purposes, it is possible to omit this one and the server will use the certificates from the `gateway{tls{}}` section. See additional advice below in TLS Entry.
...
[hashtag](#tls-entry-certificate-rotation)
...
To apply changes (even when only changing the content of those files) the server configuration needs to be reloaded. [Configuration reloadingarrow-up-right](https://docs.nats.io/running-a-nats-service/configuration#configuration-reloading) ([local](./../../../../09_chevron-right/08_configuring-nats-server-chevron-right.md#configuration-reloading))

### 

[hashtag](#gateway-behind-load-balancer)
...
[PreviousSuper-cluster with Gatewayschevron-left](https://docs.nats.io/running-a-nats-service/configuration/gateways) ([local](./../gateways.md))[NextLeaf Nodeschevron-right](https://docs.nats.io/running-a-nats-service/configuration/leafnodes) ([local](./../leafnodes.md))

...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/running-a-nats-service/configuration/leafnodes/README.md)chevron-down
...
Unlike [cluster](https://docs.nats.io/running-a-nats-service/configuration/clustering) ([local](./clustering.md)) or [gateway](https://docs.nats.io/running-a-nats-service/configuration/gateways) ([local](./gateways.md)) nodes, leaf nodes do not need to be reachable themselves and can be used to explicitly configure any acyclic graph topologies.
...
> Leaf Nodes are an important component as a way to bridge traffic between local NATS servers you control and servers that are managed by a third-party. [Synadia's NGSarrow-up-right](https://www.synadia.com/cloud) allows accounts to use leaf nodes, but gain accessibility to the global network to inexpensively connect geographically distributed servers or small clusters.

[LeafNode Configuration Options](https://docs.nats.io/running-a-nats-service/configuration/leafnodes/leafnode_conf) ([local](./leafnodes/leafnode-conf.md))

## 

[hashtag](#leafnode-configuration-tutorial)
...
Some environments prefer not to want a server that is configured to accept TLS connections for leafnodes having any traffic sent in clear text. It was possible to by-pass this using a websocket connection. However, if websocket is not desired, the accepting and remote servers can be [configured](https://docs.nats.io/running-a-nats-service/configuration/leafnodes/leafnode_conf#tls-block) ([local](./leafnodes/leafnode-conf.md#tls-block)) to perform a TLS handshake before sending the INFO protocol message.

[PreviousConfigurationchevron-left](https://docs.nats.io/running-a-nats-service/configuration/gateways/gateway) ([local](./gateways/gateway.md))[NextConfigurationchevron-right](https://docs.nats.io/running-a-nats-service/configuration/leafnodes/leafnode_conf) ([local](./leafnodes/leafnode-conf.md))

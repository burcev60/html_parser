...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/using-nats/nats-tools/nats_cli/natsbench.md)chevron-down
...
block-quoteOn this pageblock-quote
  ...
  3. [nats](https://docs.nats.io/using-nats/nats-tools/nats_cli) ([local](./../nats-cli.md))
...
Prerequisites
  ...
  * [Install the NATS CLI Tool](https://docs.nats.io/using-nats/nats-tools/nats_cli) ([local](./../nats-cli.md))
...
[hashtag](#start-the-nats-server-with-monitoring-enabled)
...
[hashtag](#run-a-publisher-throughput-test)
...
[hashtag](#run-a-publish-subscribe-throughput-test)
...
[hashtag](#run-a-1-n-throughput-test)
...
[hashtag](#run-a-n-m-throughput-test)
...
[hashtag](#run-a-request-reply-latency-test)
...
[hashtag](#run-jetstream-benchmarks)
...
[hashtag](#measure-jetstream-publication-performance)
...
[hashtag](#measure-jetstream-consumption-replay-performance)
...
Once you have stored some messages on a stream you can measure the replay performance in multiple ways:
  ...
  * `nats bench js ordered` uses an ordered _ephemeral_ consumer to receive the messages (so each client gets its own copy of the messages).

  * `nats bench js consume` uses the `Consume()` (callback) function on a _durable_ consumer to receive the messages.

  * `nats bench js fetch` uses the `Fetch()` function on a _durable_ consumer to receive messages in batches.

  * `nats bench js get` gets the messages directly by sequence number (either synchronously one by one or using 'batched gets') _without using a consumer_.
...
[hashtag](#measuring-publication-and-consumption-together)
...
[hashtag](#measure-kv-performance)
...
[hashtag](#play-around-with-the-knobs)
...
[hashtag](#leave-no-trace-clean-up-the-resources-when-you-are-finished)
...
[Previousnatschevron-left](https://docs.nats.io/using-nats/nats-tools/nats_cli) ([local](./../nats-cli.md))[Nextnkchevron-right](https://docs.nats.io/using-nats/nats-tools/nk) ([local](./../nk.md))

...
[githubEdit](https://github.com/nats-io/nats.docs/blob/master/reference/nats-protocol/nats-protocol/README.md)chevron-down
...
If the server supports _Lame Duck Mode_ notifications, and the current server has transitioned to lame duck, `ldm` will be set to `true`.
...
Enable [quick replies for cases where a request is sent to a topic with no responders](https://docs.nats.io/nats-concepts/core-nats/reqreply#no-responders) ([local](./../../nats-concepts/core-nats/reqreply.md#no-responders)).
...
[hashtag](#description-7)

Description

The `HMSG` message is the same as `MSG`, but extends the message payload with headers. See also [ADR-4 NATS Message Headersarrow-up-right](https://github.com/nats-io/nats-architecture-and-design/blob/main/adr/ADR-4.md).
...
[PreviousProtocol Demochevron-left](https://docs.nats.io/reference/reference-protocols/nats-protocol-demo) ([local](./nats-protocol-demo.md))[NextDeveloping a Clientchevron-right](https://docs.nats.io/reference/reference-protocols/nats-protocol/nats-client-dev) ([local](./nats-protocol/nats-client-dev.md))

...
source: https://docs.nats.io/running-a-nats-service/environment
---

[githubEdit](https://github.com/nats-io/nats.docs/blob/master/running-a-nats-service/environment.md)chevron-down
...
Client connections and routes are permanent. Therefore a load balancer will not distribute messages from one client connections between servers, or worse (which is common when deploying on auto-config environments), you end up with redundant, but disconnected (non-clustered) servers to which the clients will randomly connect. [Advertising needs to be switched off](https://docs.nats.io/running-a-nats-service/configuration/clustering/cluster_config) ([local](./../_unsorted/running-a-nats-service/configuration/clustering/cluster-config.md)) or configured to point to the load balancer address through the [`advertise` server config](https://docs.nats.io/running-a-nats-service/configuration/clustering/cluster_config) ([local](./../_unsorted/running-a-nats-service/configuration/clustering/cluster-config.md)) option.
...
[hashtag](#virtualization-containerization)
...
What you need to remember is that when selecting the instance types and storage options for your NATS server host instances that in public clouds: _you get what you pay for_.
...
[PreviousFlagschevron-left](https://docs.nats.io/running-a-nats-service/introduction/flags) ([local](./../_unsorted/running-a-nats-service/introduction/flags.md))[NextNATS and Dockerchevron-right](https://docs.nats.io/running-a-nats-service/nats_docker) ([local](./04_nats-and-docker-chevron-right.md))
